...
def Run_DQL(folder_name):
    ...
    dqn = DQNAgent(model=model, nb_actions=num_actions, memory=memory, nb_steps_warmup=10,
                   batch_size=32, target_model_update=1e-2, policy=policy, gamma=0.995, train_interval=8)
...
def Run_ExpectedTaskDDQN(folder_name, gamma, target_model_update, time_slot_policy=9):
    tf.keras.backend.clear_session()
    model = build_model(NUM_STATE, NUM_ACTION)
...
def Run_Sarsa(folder_name):
    ...
    try:
        dqn = SARSAAgent(model=model, nb_actions=num_actions, nb_steps_warmup=10,
                         policy=policy, gamma=0.995, train_interval=1)
    except Exception as e:
        print(e)
